...
public class LogAspect
{
    ...
    public void getControllerMethodDescription(JoinPoint joinPoint, Log controllerLog, SysOperLog operLog, Object jsonResult) throws Exception
    {
        ...
        // 设置标题 TODO国际化翻译
        operLog.setTitle(getI18nTitle(controllerLog));
        // 设置操作人类别
     ...
     * 获取国际化标题
     */
    private String getI18nTitle(Log controllerLog) {
        // 1. 优先使用code进行国际化
        if (StringUtils.isNotBlank(controllerLog.code())) {
            try {
                // 获取国际化标题
                return MessageUtils.message(controllerLog.code());
            } catch (Exception e) {
                log.warn("国际化翻译失败，使用默认标题: code={}", controllerLog.code());
