   ...
   <div class="app-container">
      <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
         ...
         <el-form-item :label="$t('job.jobGroup')" prop="jobGroup">
            <el-select v-model="queryParams.jobGroup" :placeholder="$t('job.jobGroupDec')" clearable style="width: 200px">
               <el-option
         ...
         <el-form-item :label="$t('job.status')" prop="status">
            <el-select v-model="queryParams.status" :placeholder="$t('job.statusDec')" clearable style="width: 200px">
               <el-option
      ...
      <el-dialog :title="title" v-model="open" width="820px" append-to-body>
         <el-form ref="jobRef" :model="form" :rules="rules" label-width="120px">
            <el-row>
               <el-col :span="12">
                  <el-form-item :label="$t('job.jobName')" prop="jobName">
                     <el-input v-model="form.jobName" :placeholder="$t('job.jobNameDec')" />
                  </el-form-item>
               </el-col>
               <el-col :span="12">
                  <el-form-item :label="$t('job.jobGroup')" prop="jobGroup">
                     <el-select v-model="form.jobGroup" :placeholder="$t('job.jobGroupDec')">
                        <el-option
                           v-for="dict in sys_job_group"
                           :key="dict.value"
                           :label="dict.label"
               ...
               <el-col :span="24">
                  <el-form-item prop="invokeTarget">
                     <template #label>
                        <span>
                           ...
                           <el-tooltip placement="top">
                              <template #content>
                                 <div>
                                    {{ $t('job.invokeMethodDesc') }}ryTask.ryParams('ry')
                                    <br />{{ $t('job.classInvokeExample') }}com.ruoyi.quartz.task.RyTask.ryParams('ry')
                                    <br />{{ $t('job.paramDesc') }}
                                 </div>
                              </template>
                     ...
                     <el-input v-model="form.invokeTarget" :placeholder="$t('job.invokeTargetDec')" />
                  </el-form-item>
               </el-col>
               <el-col :span="24">
                  <el-form-item :label="$t('job.cronExpression')" prop="cronExpression">
                     <el-input v-model="form.cronExpression" :placeholder="$t('job.cronExpressionDec')">
                        <template #append>
               ...
               <el-col :span="12">
                  <el-form-item label="执行策略" prop="misfirePolicy">
                     <el-radio-group v-model="form.misfirePolicy">
                        <el-radio-button value="1">{{ $t('job.immediateExecution') }}</el-radio-button>
                        <el-radio-button value="2">{{ $t('job.executeOnce') }}</el-radio-button>
                        <el-radio-button value="3">{{ $t('job.giveUpExecution') }}</el-radio-button>
                     </el-radio-group>
                  </el-form-item>
               </el-col>
               <el-col :span="12">
                  <el-form-item :label="$t('job.concurrent')">
                     <el-radio-group v-model="form.concurrent">
                        <el-radio-button value="0">{{ $t('job.allowConcurrent') }}</el-radio-button>
                        <el-radio-button value="1">{{ $t('job.disallowConcurrent') }}</el-radio-button>
                     </el-radio-group>
                  </el-form-item>
               </el-col>
            </el-row>
     ...
     <el-dialog :title="$t('job.cronExpressionGenerator')" v-model="openCron" append-to-body destroy-on-close>
       <crontab ref="crontabRef" @hide="openCron=false" @fill="crontabFill" :expression="expression"></crontab>
     </el-dialog>

      <!-- 任务日志详细 -->
      <el-dialog :title="$t('job.jobDetailDesc')" v-model="openView" width="700px" append-to-body>
            ...
            <el-row>
               ...
               <el-col :span="12">
                  <el-form-item :label="$t('job.status')">
                     ...
                     <div v-else-if="form.status == 1">{{ $t('job.paused') }}</div>
                  </el-form-item>
               </el-col>
               <el-col :span="12">
                  <el-form-item :label="$t('job.concurrent')">
                     <div v-if="form.concurrent == 0">{{ $t('job.allowConcurrent') }}</div>
                     <div v-else-if="form.concurrent == 1">{{ $t('job.disallowConcurrent') }}</div>
                  </el-form-item>
               </el-col>
               <el-col :span="12">
                  <el-form-item :label="$t('job.misfirePolicy')">
                     <div v-if="form.misfirePolicy == 0">{{ $t('job.defaultPolicy') }}</div>
                     <div v-else-if="form.misfirePolicy == 1">{{ $t('job.immediateExecution') }}</div>
                     <div v-else-if="form.misfirePolicy == 2">{{ $t('job.executeOnce') }}</div>
                     <div v-else-if="form.misfirePolicy == 3">{{ $t('job.giveUpExecution') }}</div>
                  </el-form-item>
               </el-col>
            </el-row>
         </el-form>
...
const data = reactive({
  ...
  rules: {
    jobName: [{ required: true, message: proxy.$t("job.jobNameEmpty"), trigger: "blur" }],
    invokeTarget: [{ required: true, message: proxy.$t("job.invokeTargetEmpty"), trigger: "blur" }],
    cronExpression: [{ required: true, message: proxy.$t("job.cronExpressionEmpty"), trigger: "change" }]
  }
})
...
function handleStatusChange(row) {
  let text = row.status === "0" ? proxy.$t("common.start") : proxy.$t("common.stop")
  proxy.$modal.confirm(proxy.$t("common.confirm") + '"' + text + '""' + row.jobName + '"' + proxy.$t("common.task") + '吗?').then(function () {
    return changeJobStatus(row.jobId, row.status)
  }).then(() => {
    proxy.$modal.msgSuccess(text + proxy.$t("common.success"))
  }).catch(function () {
...
function handleRun(row) {
  proxy.$modal.confirm(proxy.$t("common.confirm") + '"' + proxy.$t("job.immediateExecution") + '"' + row.jobName + '"' + proxy.$t("common.task") + '吗?').then(function () {
...
function handleAdd() {
  reset()
  open.value = true
  title.value = proxy.$t("job.addTask")
}
...
function handleUpdate(row) {
  ...
  getJob(jobId).then(response => {
    form.value = response.data
    open.value = true
    title.value = proxy.$t("job.updateTask")
  })
}
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm(proxy.$t("job.confirmDeleteJob") + '"' + jobIds + '"').then(function () {
