...
const data = reactive({
  ...
  rules: {
    configName: [{ required: true, message: proxy.$t("config.configNameRequired"), trigger: "blur" }],
    configKey: [{ required: true, message: proxy.$t("config.configKeyRequired"), trigger: "blur" }],
    configValue: [{ required: true, message: proxy.$t("config.configValueRequired"), trigger: "blur" }]
  }
})
...
function handleAdd() {
  reset()
  open.value = true
  title.value = proxy.$t("config.addConfig")
}
...
function handleUpdate(row) {
  ...
  getConfig(configId).then(response => {
    ...
    open.value = true
    title.value = proxy.$t("config.updateConfig")
  })
}
    ...
    if (valid) {
      if (form.value.configId != undefined) {
        updateConfig(form.value).then(response => {
          proxy.$modal.msgSuccess(proxy.$t("config.updateSuccess"))
          open.value = false
          getList()
        })
      } else {
        addConfig(form.value).then(response => {
          proxy.$modal.msgSuccess(proxy.$t("config.addSuccess"))
          open.value = false
          getList()
        })
      }
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm(proxy.$t("config.confirmDeleteConfig") + configIds + proxy.$t("common.menuName")+'?').then(function () {
    return delConfig(configIds)
  }).then(() => {
    getList()
    proxy.$modal.msgSuccess(proxy.$t("config.deleteSuccess"))
  }).catch(() => {})
}
...
function handleRefreshCache() {
  refreshCache().then(() => {
    proxy.$modal.msgSuccess(proxy.$t("config.refreshCacheSuccess"))
  })
}
