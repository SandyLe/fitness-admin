<template>
   <div class="app-container">
      <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
         <el-form-item :label="$t('dict.dicName')" prop="dictName">
            <el-input
               v-model="queryParams.dictName"
               :placeholder="$t('dict.dicNamePlaceholder')"
               clearable
               style="width: 240px"
      ...
      <el-table v-loading="loading" :data="typeList" @selection-change="handleSelectionChange">
         ...
         <el-table-column :label="$t('common.operation')" align="center" width="160" class-name="small-padding fixed-width">
            <template #default="scope">
               <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:dict:edit']">{{$t('common.update')}}</el-button>
...
const data = reactive({
  ...
  rules: {
    dictName: [{ required: true, message: proxy.$t("dict.dictNameRequired"), trigger: "blur" }],
    dictType: [{ required: true, message: proxy.$t("dict.dictTypeRequired"), trigger: "blur" }]
  },
})
...
function handleAdd() {
  reset()
  open.value = true
  title.value = proxy.$t("dict.addDictType")
}
...
function handleUpdate(row) {
  ...
  getType(dictId).then(response => {
    ...
    open.value = true
    title.value = proxy.$t("dict.updateDictType")
  })
}
    ...
    if (valid) {
      if (form.value.dictId != undefined) {
        updateType(form.value).then(response => {
          proxy.$modal.msgSuccess(proxy.$t("dict.updateSuccess"))
          open.value = false
          getList()
        })
      } else {
        addType(form.value).then(response => {
          proxy.$modal.msgSuccess(proxy.$t("dict.addSuccess"))
          open.value = false
          getList()
        })
      }
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm(proxy.$t("dict.confirmDeleteDictType") + '"' + dictIds + proxy.$t("common.menuName") + '"？').then(function() {
...
function handleRefreshCache() {
  refreshCache().then(() => {
    proxy.$modal.msgSuccess(proxy.$t("common.refreshCacheSuccess"))
