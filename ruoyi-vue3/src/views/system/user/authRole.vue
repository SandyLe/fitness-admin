<template>
   <div class="app-container">
      <h4 class="form-header h4">{{ $t('role.basicInfo') }}</h4>
      ...
      <h4 class="form-header h4">{{ $t('role.roleInfo') }}</h4>
      <el-table v-loading="loading" :row-key="getRowKey" @row-click="clickRow" ref="roleRef" @selection-change="handleSelectionChange" :data="roles.slice((pageNum - 1) * pageSize, pageNum * pageSize)">
         <el-table-column :label="$t('common.serialNumber')" width="55" type="index" align="center">
...
function submitForm() {
  ...
  updateAuthRole({ userId: userId, roleIds: rIds }).then(response => {
    proxy.$modal.msgSuccess($t('role.authSuccess'))
    close()
  })
}
